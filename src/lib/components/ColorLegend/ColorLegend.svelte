...
<script>
   ...
   * @type { string | undefined } [title = undefined]
   */
   ...
   * @type {Array<any> | undefined} [tickValues = undefined]
   */
   ...
   * @type { string | ((d: Object) => string) | undefined} [tickFormat = undefined]
   */
   ...
   * @type { number | undefined} [ maxWidth = undefined ]
   */
   ...
   * @type { string | undefined } [swatchSpacing = undefined]
   */
  export let swatchSpacing = undefined;

  /**
   * Optional color to indicate for NA values
   * @type { string | undefined } [naFill = undefined]
   */
  ...
  function getTicks(scaleType, xScale, thresholds) {
    ...
    if (scaleType === "sequential" && !xScale.ticks) {
      ...
      if (tickFormat) return d3Range(thresholds.length);
